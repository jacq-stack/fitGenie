  ...
  onMount(() => {
    ...
    try {
      (window as any).fitgenieWorkoutBuilderStorage?.bindTextarea({
        textareaId: 'equipment',
        responseKey: 'equipmentAccess',
        questionText: 'What type of equipment do you have access to?',
        stepNumber: 5,
        saveOnIds: ['next-step']
      });
    } catch (e) {
      console.error('Failed to initialize workout builder storage binding (step 5):', e);
    }
  });
</script>
